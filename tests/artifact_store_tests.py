...
class DataStoreNode(ArtifactStoreNode):
    def __init__(self, name: str, path: str, init_state: str = "new", max_old_samples: int = None) -> None:
        super().__init__(name, path, "data_store.json", init_state, max_old_samples)
...
class TestArtifactStore(unittest.TestCase):
    ...
    def test_empty_pipeline(self):
        ...
        for i in range(5):
            create_file(f"{self.data_store_path}/test_file{i}.txt", f"test file {i}")
            #create_file(f"{self.model_registry_path}/test_model{i}.txt", f"test model {i}")
            time.sleep(1)
