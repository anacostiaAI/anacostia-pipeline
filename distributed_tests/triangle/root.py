...
class DataReadingNode(BaseActionNode):
    def __init__(self, name: str, data_store: FilesystemStoreNode, predecessors: List[BaseNode] = None, remote_successors: List[str] = None) -> None:
        if predecessors is None:
            predecessors = [data_store]
        else:
            predecessors.append(data_store)

        super().__init__(name=name, predecessors=predecessors, remote_successors=remote_successors)
        self.data_store = data_store

    def execute(self, *args, **kwargs) -> bool:
        self.log("Logging node beginning execution", level="INFO")
        ...
        for artifact_path in artifacts_paths:
            with self.data_store.load_artifact(filepath=artifact_path) as fullpath:
                with open(fullpath, "r", encoding="utf-8") as f:
                    data = f.read()
                    self.log(f"Reading data: {data}", level="INFO")

        self.log("Logging node done executing", level="INFO")
        return True
...
data_reading_node = DataReadingNode(
    "data_reading_node", 
...
# Create the pipeline
pipeline = Pipeline(name="test_pipeline", nodes=[metadata_store, data_store, data_reading_node], loggers=[logger])
