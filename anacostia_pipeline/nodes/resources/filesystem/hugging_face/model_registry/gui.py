...
from anacostia_pipeline.nodes.resources.filesystem.hugging_face.model_registry.fragments import model_card_viewer, model_registry_home, table_row, create_table_rows
...
class ModelRegistryGUI(BaseGUI):
    def __init__(self, node, host: str, port: int, metadata_store = None, metadata_store_client = None, *args, **kwargs):
        ...
        def format_file_entries(file_entries: Union[List[Dict], Dict]) -> Union[List[Dict], Dict]:
            ...
            if type(file_entries) is list:
                for file_entry in file_entries:
                    ...
                    file_entry["file_display_endpoint"] = f"{self.get_node_prefix()}/retrieve_file?file_id={file_entry['id'] + 1}"
                ...
                # the entry id of the model model card file is usually the one right after the model
                file_entry["file_display_endpoint"] = f"{self.get_node_prefix()}/retrieve_file?file_id={file_entry['id'] + 1}"
                file_entry["state_change_event_name"] = f"StateUpdate{file_entry['id']}"
                return file_entry
        ...
        async def endpoint(request: Request):
            ...
            file_entries = self.format_file_entries(file_entries)

            return model_registry_home(
                sse_endpoint = self.event_source,
                event_name = self.event_name,
                model_entries = file_entries,
            ) 
        ...
        async def sample(request: Request, file_id: int):
            ...
            # Render markdown (including embedded HTML) as html
            html = markdown.markdown(body, extensions=['extra', 'toc', 'nl2br', "fenced_code"])

            # wrap inside model markdown-div 
            return model_card_viewer(markdown_html_str=html)